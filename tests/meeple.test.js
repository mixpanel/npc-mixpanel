	...
	describe('Sequence Execution', () => {
      ...
      `);
			...
			const sequenceSpec = {
				...
				temperature: 10,
				'chaos-range': [10, 10],
				actions: [{ action: 'fillOutForm', selector: '[role=radiogroup]', clicksPerGroup: 2 }]
			};
		...
		test('validateSequence accepts fillOutForm action type', () => {
			const validSequence = {
				...
				temperature: 8,
				actions: [{ action: 'fillOutForm', selector: '[role=radiogroup]', clicksPerGroup: 5 }]
			};
	...
	describe('Hot Zones - Form Detection', () => {
      ...
      `);
			...
			console.log(
				...
				formHotZones.map(z => ({ tag: z.tag, type: z.formType, role: z.ariaRole, priority: z.priority }))
			);
